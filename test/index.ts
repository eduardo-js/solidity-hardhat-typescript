...
describe("VoidToken", function () {
  it("Should return the name of the coin", async function () {
    const voidToken = await (
      await ethers.getContractFactory("VoidToken")
    ).deploy();
  ...
  it("Should Transfer 50 coins to a account", async function () {
    ...
    expect(await voidToken.balanceOf(owner.address)).to.be.equals(
      Number(ownerBalance) - Number(fifty)
    );
  });
});
